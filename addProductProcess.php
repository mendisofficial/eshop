...
Database::iud("INSERT INTO `product` (`price`, `qty`, `description`, `title`, `datetime_added`, `delivery_fee_colombo`, `delivery_fee_other`, `category_cat_id`, `model_has_brand_id`, `color_clr_id`, `status_status_id`, `condition_condition_id`, `users_email`) VALUES ('". $cost . "', '" . $qty . "', '" . $desc . "', '" . $title . "', '" . $date . "', '" . $dwc . "', '" . $doc . "', '" . $category . "', '" . $mhb_id . "', '" . $clr . "', '" . $status . "', '" . $condition . "', '" . $email . "');");
...
if($length <= 3 && $length > 0){
    $allowed_image_extension = array("image/jpg", "image/jpeg", "image/png", "image/svg+xml");
    ...
    for($x = 0; $x < $length; $x++){
        if(isset($_FILES["img".$x])){
            ...
            if(in_array($file_extention, $allowed_image_extension)){
                ...
                $file_name = "resources//products//" . $title . "_" . $x . "_" . uniqid() . $new_image_extention;
